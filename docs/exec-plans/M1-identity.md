...
Deliver the first visibly product-like KeylorForge slice on a real phone while preserving ADR-002: Supabase Auth owns identity/token issuance and FastAPI owns application authorization.
...
- after deletion, access tokens and refresh credentials issued before deletion cannot regain protected access or recreate application state, and a new password login for the deleted identity cannot restore the deleted KeylorForge account under the accepted final semantics;
...
4. KeylorForge application user/profile data is owned by the application database and linked deterministically to the external auth subject.
...
7. Account deletion has an explicit terminal identity state: valid-looking credentials issued before deletion must not be able to reprovision or regain a deleted KeylorForge account.
