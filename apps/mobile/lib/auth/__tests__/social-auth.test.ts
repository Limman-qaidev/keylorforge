...
function browserResult(
  value: { type: 'cancel' | 'dismiss' } | { type: 'success'; url: string },
...
describe('social auth', () => {
  ...
  it.each([
    ...
    jest
      .mocked(WebBrowser.openAuthSessionAsync)
      .mockResolvedValue(browserResult({ type: 'success', url: callbackUrl }));
  ...
  it.each([
    ...
    'installs a successful query/hash callback through setSession',
    async (url) => {
      const client = createClient();
      jest
        .mocked(WebBrowser.openAuthSessionAsync)
